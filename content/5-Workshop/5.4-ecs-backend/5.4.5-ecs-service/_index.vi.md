...
# 5.4.5. Định nghĩa ECS Task, Khởi tạo Service và Kết nối ALB
...
4. Tại phần **Container - 1**:
   ...
   - **Port mappings**: 
     - **Container port**: `80` (Hoặc port mà code Backend của bạn đang lắng nghe).
...
6. **Load balancing**: 
   ...
   - Target group: Chọn **Use an existing target group** và chọn `Eshop-Backend-TG` (đã tạo ở bài 5.4.4).
...
2. Khi trạng thái các Task chuyển sang **Running**, hãy lấy **DNS Name** của ALB (mà bạn đã lưu ở bài 5.4.4).
3. Mở trình duyệt mới, dán đường dẫn DNS của ALB vào và nhấn Enter. 
   - Nếu bạn thấy trang web hoặc phản hồi từ API Backend của mình, **XIN CHÚC MỪNG!** Hệ thống Backend Container của bạn đã hoạt động hoàn hảo!